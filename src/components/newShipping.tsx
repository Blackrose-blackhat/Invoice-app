...
const NewShipping = () => {
  ...
  useEffect(() => {
    const getShipping = async () => {
      ...
      try{
        const billingData = await getAllBillings();
        setFormData((prevFormData) => ({
          ...prevFormData,
          billing: billingData,
        }));
  ...
  const handleSelectChange = (type: any, value: any) => {
    ...
    const selectedBillling = formData.billing.find((bill)=> bill.billingCompany === value );


    setFormData((prevState) => ({
      ...prevState,
      [type]: value,
      clientName:selectedBillling? selectedBillling.clientName : "",
      clientID : selectedBillling? selectedBillling.clientID : "",
      selectedBilling:selectedBillling? selectedBillling : "",
    }));
  ...
  return (
    <div className="flex flex-col gap-40 p-10 w-full">
      ...
      <form
        ...
        className="flex flex-col w-full h-fit overflow-auto mb-5 mt-5   gap-5 bg-white shadow-sm shadow-neutral-700 p-2 text-sm  "
      >
        <div className="w-full flex flex-row gap-5 ">
          ...
          <div className="flex flex-col gap-3 w-1/2">
            <div className="flex flex-col gap-2">
              ...
              <select
                onChange={(e) =>
                  handleSelectChange("billingCompanyName", e.target.value)
                }
                value={formData.billingCompanyName}
                placeholder="select Billing Company Name"
                className="border-2 border-gray-300 p-2 rounded-md w-full bg-transparent outline-none"
              >
                <option value="">Select billing company Name</option>
                {formData.billing.map((bill) => (
                  <option key={bill._id} value={bill.billingCompany}>
                    {bill.billingCompany}
                  </option>
                ))}
              </select>
            ...
            <div className="flex flex-row gap-2 items-center mt-8">
              ...
              <button
                type="submit"
                className="p-2 rounded-md bg-green-400 text-sm font-semibold text-white"
              >
                Save
              </button>
            </div>
